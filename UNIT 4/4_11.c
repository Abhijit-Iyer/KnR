//Reverse Polish Calculator with getop() modified to eliminate ungetch() from the code
...
int getop(char s[])
{
	...
	if(isalpha(s[i]))
		while(isalpha(s[++i] = c = getch()))	//Checks for the presence of alphabets in the experssion
			;
	...
	if( c!= EOF)
		temporary_buffer[++buffer_index] = c;
...
char getch(void)
{
	return (buffer_index > 0) ? temporary_buffer[buffer_index--] : getchar();
}
